...
export default (content) => {
  ...
  return {
    ...
    init() {
      if (!editor) {
        // Remove any existing editors before initializing a new one to prevent duplicates.
        // The browser cache retains the previous state, but tiptap reinitializes the editor from cache without first clearing existing instances.
        // on htmx boosted pages.
        let textContents = [];
        document.querySelectorAll('.tiptap.ProseMirror').forEach((el) => {
          textContents.push(el.innerHTML);
          el.remove();
        });
        // set content to the last editor content
        if (textContents.length > 0) {
          content = textContents.join('\n');
        }

        this.initializeEditor(this.$refs.elementEditor);
      }
    },
    ...
    updateHistoryStatus() {
      ...
      this.updatedAt = Date.now(); // Force Alpine to rerender
    },
    undo() {
      if (this.canUndo) {
